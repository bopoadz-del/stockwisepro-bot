...
export function LiveMarketData() {
  ...
  const fetchIndices = async () => {
    setLoading(true);
    try {
      const res = await stocksApi.getIndices();
      if (res.data && res.data.length > 0) {
        setIndices(res.data.map((q: any) => ({
          symbol: q.symbol,
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {loading && indices.length === 0 ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-[#1f1f1f] rounded-xl border border-white/10 p-5 animate-pulse">
              ...
              <ScrollReveal key={index.symbol} delay={i * 0.1}>
                ...
                >
                  ...
                  >
                    ...
                    <span>({formatPercentage(index.changesPercentage)})</span>
                  </div>
                  {/* Mini Sparkline */}
